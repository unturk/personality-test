...
import com.galaksiya.db.gquery.OrderBy;
	...
	public Response getQuestions(@QueryParam(CATEGORY) Integer categoryId, @Context HttpServletRequest request) {
		...
		try {
			GQueryFilter filter = new GQueryFilter(
				new GQueryFilterPart("category.id").setOperator(Operators.EQUAL, categoryId)).setOrderBy(ID, OrderBy.ASC);
			List<Question> questions = DatabaseConnector.getInstance().getObjects(filter, Question.class, GET_CHILD);
			if (questions != null && !questions.isEmpty()) {
				response = RestUtils.createResponseOK(EntityUtils.convertEntityToJsonArray(questions));
				log.succeed();
			} else {
